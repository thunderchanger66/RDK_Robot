...
class PanTilt:
    def __init__(self, ZDTMotorController,PanTiltMotor):
        ...
        初始化二维云台对象
        :param ZDTMotorController: 电机控制器类
        :param pan_addr: 水平旋转电机地址 (int)
        :param tilt_addr: 垂直旋转电机地址 (int)
        :param resolution: 每圈的脉冲数 (默认为16细分，即3200脉冲/圈) (int)
        """
        self.motor_controller = ZDTMotorController('/dev/ttyS1', 115200)
    ...
    def move_pan(self, angle, duration, acceleration=10):
        """
        设置水平旋转电机的角度
        :param angle: 目标角度 (度) (float)
        ...
        :param acceleration: 加速度 (RPM/s) (int)
        """
        velocity = angle / duration  # 计算速度 (度/秒)
        self.pan_motor.set_angle(angle, velocity, acceleration, raF=False, snF=False)

    def move_tilt(self, angle, duration, acceleration=10):
        """
        设置垂直旋转电机的角度
        :param angle: 目标角度 (度) (float)
        :param duration: 运动时间 (秒) (float)
        ...
        """
        velocity = angle / duration  # 计算速度 (度/秒)
        self.tilt_motor.set_angle(angle, velocity, acceleration, raF=False, snF=False)
    ...
    def move_to_position(self, pan_angle, tilt_angle, duration, acceleration=10):
        ...
        :param acceleration: 加速度 (RPM/s) (int)
        """
        self.move_pan(pan_angle, duration, acceleration)
        self.move_tilt(tilt_angle, duration, acceleration)
...
if __name__ == '__main__':
    ...
    # 移动到指定位置
    chassis.move_to_position(x_distance=0, y_distance=0, z_angle=180, duration=2.0)
...
#     # 移动到指定位置
#     pan_tilt.move_to_position(pan_angle=45.0, tilt_angle=20.0, duration=2.0, acceleration=10)
