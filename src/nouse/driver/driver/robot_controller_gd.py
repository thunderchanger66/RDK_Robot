...
创建ROS节点，用来接收光流计，IMU，主节点的命令  六个电机同步发布以防串口冲突

TO DO
接入光流节点，接入IMU节点，接入主节点
调试PID
验证ROS控制
给云台添加零点位置然后限幅
...
class ZDTMotorController:
    ...
    def Emm_V5_Reset_CurPos_To_Zero(self, addr):
        ...
        self.send_command(cmd)
        return(self.Emm_V5_Receive_Data())
...
class Wheel:
    ...
    def calculate_velocity_from_rpm(self, rpm):
        """
        根据转速 (RPM) 计算线速度 (米/秒)
        :param rpm: 转速 (RPM) (float)
        :return: 线速度 (米/秒) (float)
        """
        circumference = self.diameter * 3.14159  # 轮子周长 (米)
    ...
    def parse_speed_from_response(self, hex_data):
        """
        解析协议返回的十六进制字符串，提取电机转速
        :param hex_data: 形如 '01 35 01 05 dc 6b' 的字符串
        :return: 实际转速（int，单位RPM），解析失败返回None
        """

        bytes_list = [int(b, 16) for b in hex_data.strip().split()]


        if bytes_list[0] == self.addr and bytes_list[1] == 53:
            vel = (bytes_list[3] << 8) | bytes_list[4]
            Motor_Vel = -vel if bytes_list[2] else vel

            if(self.cw_positive == False):
                Motor_Vel = -Motor_Vel
    ...
    def get_motor_speed(self):
        ...
        self.motor_controller.Emm_V5_Read_Sys_Params(self.addr, 'S_VEL')
        data =  self.motor_controller.Emm_V5_Receive_Data()
        hex_data = data[0]
        rmp = self.parse_speed_from_response(hex_data)
        vel = self.calculate_velocity_from_rpm(rmp)  
        return vel 
...
class Chassis:
    ...
    def move_in_velocity(self, x_velocity, y_velocity, z_velocity, acceleration=30):
        """
        以一定速度移动
    ...
    def wheel_speeds_to_chassis_velocity(self, wheel_speeds):
        """
        由四个轮子的线速度（单位m/s）计算底盘速度（vx, vy, wz）
        :param wheel_speeds: [v1, v2, v3, v4]，顺序为LF, RF, LR, RR
        :return: (vx, vy, wz)
        """
        L = self.wheelbase
        W = self.track_width

        v1, v2, v3, v4 = wheel_speeds
        vx = (v1 + v2 + v3 + v4) / 4
        vy = (-v1 + v2 + v3 - v4) / 4
        wz_rad = (-v1 + v2 - v3 + v4) / (4 * (L/2 + W/2))
        wz_deg = wz_rad * 180 / math.pi          # °/s
        return vx, vy, wz_deg
    

    def get_car_speed(self):
        wheel_speeds = [wheel.get_motor_speed() for wheel in self.wheels]  # 假设get_speed返回m/s
        vx, vy, wz = self.wheel_speeds_to_chassis_velocity(wheel_speeds)
        vx_huajian =round(vx,3)
        vy_huajian =round(vy,3)
        wz_huajian =round(wz,3)
        return vx_huajian, vy_huajian, wz_huajian
...
if __name__ == '__main__':
    ...
    # chassis.move_in_velocity(0,0,0,acceleration=30)
    # chassis.move_to_position(0.1, 0.1, 90, acceleration=30,duration=2)

    # chassis.start_to_motion()

    while(1):
        print(chassis.get_car_speed())
